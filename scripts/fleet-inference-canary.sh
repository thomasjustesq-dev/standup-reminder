...
#   grok → openai (Codex) → kimi → gemini
# Anthropic is not a Copilot BYOK rung.
...
ORDER=(
  ...
  "openai|Codex (OpenAI)|probe_openai"
...
probe_openai() {
  ...
  OPENAI_COMPAT_MODEL="${OPENAI_INFERENCE_MODEL:-gpt-5.6-sol}" \
...
probe_kimi() {
  ...
  OPENAI_COMPAT_MODEL="${KIMI_INFERENCE_MODEL:-kimi-k2.7-code}" \
...
summary_line ""
summary_line "Preference order: **grok → openai → kimi → gemini**"
summary_line ""
...
if [[ "$configured_count" -eq 0 ]]; then
  echo "::error::No fleet inference secrets are configured (XAI/OPENAI/MOONSHOT|KIMI/GEMINI)."
  exit 1
fi
