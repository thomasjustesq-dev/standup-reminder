...
echo "$order_line" | rg -q 'grok → openai → kimi → gemini' \
...
expected="grok openai kimi gemini"
got="${ids[*]}"
...
if env -u XAI_API_KEY -u GEMINI_API_KEY -u MOONSHOT_API_KEY -u KIMI_API_KEY \
    -u OPENAI_API_KEY -u ANTHROPIC_API_KEY -u CLAUDE_CODE_OAUTH_TOKEN \
